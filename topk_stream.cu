...
void doc_query_scoring_gpu_function(std::vector<std::vector<uint16_t>> &querys,
    ...
    // stream
    int n_stream = querys.size();
    cudaStream_t *q_streams = (cudaStream_t *)malloc(n_stream * sizeof(cudaStream_t));
    int stream_id = 0;
    ...
    for (auto &query : querys) {
        ...
        cudaStreamCreate(&q_streams[stream_id]);
        ...
        docQueryScoringCoalescedMemoryAccessSampleKernel<<<grid, block, 0, q_streams[stream_id]>>>(d_docs,
                                                                                                   d_doc_lens, n_docs, d_query, query_len, d_scores);
        // cudaDeviceSynchronize();
        cudaMemcpyAsync(scores.data(), d_scores, sizeof(float) * n_docs, cudaMemcpyDeviceToHost, q_streams[stream_id]);
