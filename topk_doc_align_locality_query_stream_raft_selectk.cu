...
void doc_query_scoring_gpu(std::vector<std::vector<uint16_t>> &querys,
    ...
    uint16_t *d_docs = nullptr, *d_in_docs = nullptr;
...
#pragma omp parallel for schedule(static)
    ...
    for (int i = 0; i < n_querys; i++) {
        ...
        cudaHostRegister(querys[i].data(), sizeof(uint16_t) * querys[i].size(), cudaHostRegisterDefault);
    }
...
#pragma omp parallel for schedule(static)
    ...
    std::cout << "async cost " << std::chrono::duration_cast<std::chrono::milliseconds>(lt2 - lt).count() << " ms " << std::endl;

    for (int i = 0; i < n_querys; i++) {
        cudaStreamSynchronize(q_streams[i]);
        cudaHostUnregister(querys[i].data());
    }
    std::chrono::high_resolution_clock::time_point lt1 = std::chrono::high_resolution_clock::now();
    std::cout << "stream sync launch kernel cost " << std::chrono::duration_cast<std::chrono::milliseconds>(lt1 - lt).count() << " ms " << std::endl;
    ...
    for (int stream_id = 0; stream_id < n_querys; stream_id++) {
        ...
        auto in_span = raft::make_mdspan<const float, int64_t, raft::row_major, false, true>(d_querys_scores + stream_id * n_docs, in_extent);
