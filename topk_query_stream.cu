...
void doc_query_scoring_gpu_function(std::vector<std::vector<uint16_t>> &querys,
    ...
    for (int i = 0; i < n_querys; i++) {
        ...
        std::cout << "partial_sort cost " << std::chrono::duration_cast<std::chrono::milliseconds>(tt1 - tt).count() << " ms " << std::endl;

        std::vector<int> s_ans(s_indices.begin(), s_indices.begin() + topk);
        indices.push_back(s_ans);
    }
    std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
    std::cout << "total partial_sort cost " << std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t).count() << " ms " << std::endl;
